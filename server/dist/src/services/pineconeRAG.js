...
async function generateQueryEmbedding(query) {
    ...
    }); // using 'as any' just in case the types are missing the parameter, though it worked in testing
 ...
 */
async function queryPinecone(embedding, topK = 5, filters) {
    if (!pineconeIndex)
 ...
 */
function extractSchemeFromMetadata(id, metadata, score) {
 ...
 */
function buildGroundedContext(schemes) {
    if (schemes.length === 0)
        return 'NO SCHEMES FOUND IN DATABASE.';
    return schemes
        .map((s, i) => `
 ...
 */
async function generateGroundedResponse(userQuery, context, schemes) {
    if (!genAI) {
...
Respond with the JSON object as specified. Do not include any text outside the JSON.`;
    ...
    try {
        ...
        const enrichedSchemes = schemes.map((scheme) => {
            const relevance = parsed.schemeRelevance?.find((r) => r.schemeName === scheme.schemeName);
            return {
                ...scheme,
 ...
 */
function buildFallbackResponse(schemes) {
    if (schemes.length === 0) {
        return {
 ...
 */
async function querySchemes(userQuery, filters) {
    console.log(`\n[RAG] ════════════════════════════════════════`);
    ...
    try {
        ...
        const { reply, schemes: enrichedSchemes, suggestions } = await generateGroundedResponse(userQuery, context, schemes);
        const debugInfo = {
