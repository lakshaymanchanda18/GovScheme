...
exports.sendEligibilityNotificationEmail = sendEligibilityNotificationEmail;
/**
...
// ─── BASE TEMPLATE ────────────────────────────────────────────
const BASE_TEMPLATE = handlebars_1.default.compile(`
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
// ─── EMAIL TEMPLATES ──────────────────────────────────────────
const TEMPLATES = {
    welcome: handlebars_1.default.compile(`
  ...
  `),
    applicationSubmitted: handlebars_1.default.compile(`
  ...
  `),
    statusUpdate: handlebars_1.default.compile(`
  ...
  `),
    eligibilityNotification: handlebars_1.default.compile(`
 ...
 */
async function renderAndSend(payload) {
...
function queueEmail(payload) {
    (0, queue_1.enqueueJob)('EMAIL_SEND', payload);
}
/**
 * Send an email immediately (blocking).
 */
async function sendEmailNow(payload) {
    try {
...
function sendApplicationSubmittedEmail(to, firstName, schemeName, applicationId) {
    queueEmail({
        to,
...
function sendStatusUpdateEmail(to, firstName, schemeName, status, rejectionReason) {
...
function sendEligibilityNotificationEmail(to, firstName, schemeName, schemeId, matchScore, benefits) {
    queueEmail({
        to,
...
(0, queue_1.registerHandler)('EMAIL_SEND', async (payload) => {
