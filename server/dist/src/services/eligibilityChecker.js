...
exports.eligibilityChecker = exports.EligibilityChecker = void 0;
...
function parseAgeLimit(ageLimit) {
    if (!ageLimit)
        return {};
...
function parseIncomeLimit(incomeLimit) {
...
class EligibilityChecker {
    ...
    async checkEligibility(userId, schemeId) {
        ...
        // Persist the check
        await prisma_1.prisma.eligibilityCheck.create({
            data: {
                userId,
                schemeId,
    ...
    async checkAllSchemes(userId) {
        ...
        const results = schemes.map((scheme) => this.evaluate(user, scheme));
    ...
    evaluate(user, scheme) {
        ...
        if (scheme.stateSpecific) {
            ...
            const schemeStates = scheme.stateSpecific.toLowerCase().split(',').map((s) => s.trim());
            if (user.state) {
                const userState = user.state.toLowerCase().trim();
                const stateMatch = schemeStates.some((s) => s.includes(userState) || userState.includes(s) || s === 'all');
                if (stateMatch) {
    ...
    buildExplanation(schemeName, matched, unmatched, score, isEligible) {
        const parts = [];
        if (isEligible) {
    ...
    buildRecommendations(unmatched) {
        const recs = [];
...
exports.eligibilityChecker = new EligibilityChecker();
